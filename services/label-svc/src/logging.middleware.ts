...
export class LoggingMiddleware implements NestMiddleware {
  use(req: any, res: any, next: () => void) {
    ...
    // Extract shipmentId if label routes carry it: e.g., /v1/labels/:shipmentId
    let shipmentId = '';
    const m = (req.originalUrl || req.url || '').match(/\/v1\/(labels|shipments)\/([^/]+)/);
    if (m && m[2]) shipmentId = m[2];
    ...
    res.on('finish', () => {
      ...
      const service = process.env.OTEL_SERVICE_NAME || process.env.npm_package_name || 'label-svc';
